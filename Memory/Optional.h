...
#define optional_ALIGN_AS( to_align ) \
    typename type_of_size< alignment_types, alignment_of< to_align >::value >::type
...

            {
                ...
                typedef typename conditional<N == sizeof( typename List::head ),typename List::head,typename type_of_size<typename List::tail, N >::type >::type type;
...
/// C++03 constructed union to hold value.
            ...
            private:
                ...
                value_type const & GetValue() const optional_ref_qual
                ...
                value_type & GetValue() optional_ref_qual
                ...
                value_type const && GetValue() const optional_refref_qual
                ...
                value_type && GetValue() optional_refref_qual
...
/// optional
        ...
        public:
            ...
            Optional( Optional const & rhs ): has_value_( rhs.HasValue() )

            {

                if ( rhs.HasValue() )
                    contained.construct_value( rhs.contained.GetValue() );

            }

#if optional_CPP11_OR_GREATER

            optional_constexpr14 Optional( Optional && rhs ) noexcept( std::is_nothrow_move_constructible<T>::value ): has_value_( rhs.HasValue() )
            ...
            template< class... Args >
            optional_constexpr explicit Optional( nonstd_lite_in_place_type_t(T), Args&&... args ): has_value_( true ), contained( T( std::forward<Args>(args)...) )
            {}


            template< class U, class... Args >
            optional_constexpr explicit Optional( nonstd_lite_in_place_type_t(T), std::initializer_list<U> il, Args&&... args ): has_value_( true ), contained( T( il, std::forward<Args>(args)...) )
            {}
...

            {
                ...
                else if ( HasValue() == false && rhs.HasValue() == true  ) initialize( *rhs );
                else if ( HasValue() == true  && rhs.HasValue() == true  ) contained.GetValue() = *rhs;
                return *this;
            ...
            template< class U,typename = typename std::enable_if< std::is_same< typename std::decay<U>::type, T>::value >::type >

            Optional & operator=( U && v )

            {

                if ( HasValue() ) contained.GetValue() = std::forward<U>( v );
...

            {
                ...
                if      ( HasValue() == true  && rhs.HasValue() == true  ) { swap( **this, *rhs ); }

                else if ( HasValue() == false && rhs.HasValue() == true  ) { initialize( *rhs ); rhs.reset(); }

                else if ( HasValue() == true  && rhs.HasValue() == false ) { rhs.initialize( **this ); reset(); }
...

            {
                ...
                return assert( HasValue() ),contained.value_ptr();
...

            {
                ...
                return assert( HasValue() ),contained.GetValue();
...

            {
                ...
                return HasValue() ? &optional::this_type_does_not_support_comparisons : 0;

            }
#endif


            optional_constexpr bool HasValue() const optional_noexcept
            ...
            optional_constexpr14 value_type const & GetValue() const optional_ref_qual
            ...
            optional_constexpr14 value_type & GetValue() optional_ref_qual
            ...
            optional_constexpr14 value_type const && GetValue() const optional_refref_qual
            ...
            optional_constexpr14 value_type && GetValue() optional_refref_qual
...

            {
                ...
                return HasValue() ? contained.GetValue() : static_cast<T>(std::forward<U>( v ) );
...

            {
                ...
                return HasValue() ? contained.GetValue() : static_cast<value_type>( v );
...
namespace std
{
    ...
    /*
    template< class T >
    class hash< PDP::Optional<T> >

    {

    public:

        std::size_t operator()( PDP::Optional<T> const & v ) const optional_noexcept
