...
inline void* aligned_malloc(size_t required_bytes, size_t alignment)
{
    ...
    long offset = alignment - 1 + sizeof(void*);
...
#else
    #define AlignedMalloc( size, alignment ) aligned_malloc( size, alignment )
    #define AlignedFree( pMemory ) aligned_free( pMemory )
#endif
...
template< typename T > class PlacementNewLinkedList
	: public Memory::PlacementNewCreationHelper< T >
	, public Memory::AllocationTracker
{
public:
...
public:
	...
	void FreeAll()
	{
		...
		for( typename std::vector< Chunk >::iterator it = chunks.begin(); it != chunks.end(); ++it )
		{
...
protected:
	~CompactableChunkPolicy()
	{
		for( typename Chunks::iterator it = chunks.begin(); it != chunks.end(); ++it )
		{
			it->Release();
		}
	}
	...
	void* Pop()
	{
		if( allocChunk == 0 || allocChunk->blocksAvailable == 0 )
		{
			...
			typename Chunks::iterator it = chunks.begin();
			for(;; ++it)
			{
				if( it == chunks.end() )
				{
...
public:
	...
	unsigned int GetNumBlocksAllocated() const
	{
		return Memory::SharedFreeList::GetNumBlocksAllocated( sizeof(T) );
	}

	unsigned int GetNumberOfBlocksInUse() const
	{
		return Memory::SharedFreeList::GetNumBlocksInUse( sizeof(T) );
	}

	unsigned int GetPeakNumberOfBlocksInUse() const
	{
		return Memory::SharedFreeList::GetPeakNumberOfBlocksInUse( sizeof(T) );
	}
...
protected:
	...
	void Push( void* pItem )
	{
		Memory::SharedFreeList::Deallocate( pItem, sizeof(T) );
	}

	void* Pop()
	{
		return Memory::SharedFreeList::Allocate( sizeof(T) );
	}

	void Grow( unsigned int numBlocks )
	{
		Memory::SharedFreeList::Grow( numBlocks, sizeof(T) );
	}

	// This class only works on objects that fit inside one of SharedFreeList's internal lists
	STATIC_CHECK( sizeof(T) <= Memory::SharedFreeList::maxBytes,
