...
//	Initializer.
CDynamicProgrammingAlg::CDynamicProgrammingAlg(CMap &rMap, int goalRow, int goalCol) : m_map(rMap),m_bestAction(WIDTH,HEIGHT)
{
    m_goalRow = goalRow;
    m_goalCol = goalCol;
    for (int row = 0; row < WIDTH; row++) {
...
//	Initializer
CMap::CMap(const bool rMap[WIDTH][HEIGHT], const float rCosts[WIDTH][HEIGHT], CAction &rActionProbs):m_walls(WIDTH,HEIGHT),m_costs(WIDTH,HEIGHT)
{
