...
#include "TypeTraits/IsNullPtr.h"
#include "TypeTraits/IsVoid.h"
#include "TypeTraits/IsArray.h"
#include "TypeTraits/IsSame.h"
#include "TypeTraits/AddPointer.h"
#include "TypeTraits/Conditional.h"
#include "TypeTraits/Decay.h"
#include "TypeTraits/IsConst.h"
#include "TypeTraits/IntegerSequence.h"
#include "TypeTraits/Limit.h"
#include "TypeTraits/Declval.h"
#include "TypeTraits/RemoveConst.h"
#include "TypeTraits/RemoveAllExtents.h"
#include "TypeTraits/IsDestructible.h"
#include "TypeTraits/IsAssignable.h"
...
namespace Detail
{
    ...
    struct would_call_std_swap_impl {
        ...
        template<typename U>
        static auto check(int)
        -> LD::Detail::IntegralConstant<bool, PDP::IsSame<decltype( PDP::Swap(Declval<U&>(), Declval<U&>())), tag>::value>;
    ...
    struct is_constructible_1 {
        ...
        static yes& test(int(*)[sizeof(typename TypeIdentity<U>::type(static_cast<Arg_>(*((typename PDP::RemoveReference<Arg_>::type*)0))))]);
        template<typename U, typename Arg_>
        static no& test(...);
        enum {
    ...
    struct is_constructible_1<T*, U*>
            : Conditional<
                    IsVoid<typename PDP::RemoveConst<T>::type>::value,
                    LD::Detail::IntegralConstant<bool, true>,
                    typename Conditional<
                            IsVoid<typename PDP::RemoveConst<U>::type>::value,
    ...
    struct is_constructible_2 {
        ...
        sizeof(typename TypeIdentity<U>::type(
                static_cast<Arg1_>(*((typename PDP::RemoveReference<Arg1_>::type*)0)),
                static_cast<Arg2_>(*((typename PDP::RemoveReference<Arg2_>::type*)0))
        ))
        ]);
        template<typename U, typename Arg1_, typename Arg2_>
        static no& test(...);
...
#ifndef _LIBCPP_HAS_NO_RVALUE_REFERENCES
        : public IsConstructible<_Tp, typename PDP::AddRvalueReference<_Tp>::type>
#else
...
namespace Detail
{
    ...
    void CopyTupleImpl(T1 const & from, T2 & to, PDP::IndexSequence<I...>)
    {
    ...
    void CopyTupleFromTo(T1 const & from, T2 & to)
    {
        CopyTupleImpl(
                from, to,
                typename PDP::MakeIndexSequence<TupleSize<T1>::value>::type{});
    }
    ...
    auto tuple_cat1(Tuple1&& tup1, Tuple2&& tup2,
                    PDP::IndexSequence<Indices1...>, PDP::IndexSequence<Indices2...>)
    {
        return MakeTuple(
                Get<Indices1>(PDP::Forward<Tuple1>(tup1))...,
                Get<Indices2>(PDP::Forward<Tuple2>(tup2))...
        );
    }
    template<typename T, typename U, LD::UInteger ... INDEX>
    constexpr auto TupleAreEqualImpl(const T & a, const U & b, PDP::IndexSequence<INDEX...> )-> bool
    {
        return (... && (Get<INDEX>(a) == Get<INDEX>(b)));
    }

    template<typename T, typename U, LD::UInteger ... INDEX>
    constexpr auto TupleAreLessImpl(const T & a, const U & b, PDP::IndexSequence<INDEX...> )-> bool
    {
        return (... && (Get<INDEX>(a) < Get<INDEX>(b)));
    }
    ...
    {
        return Detail::TupleAreEqualImpl(a, b, typename PDP::MakeIndexSequence<TupleSize<typename PDP::Decay<T>::type>::value>::type{});
    }
    ...
    {
        return Detail::TupleAreLessImpl(a, b, typename PDP::MakeIndexSequence<TupleSize<typename PDP::Decay<T>::type>::value>::type{});
    }


    template<typename T, typename U, LD::UInteger ... Index>
    void TupleSwapImpl(T & a, U & b, PDP::IndexSequence<Index...> )
    {
        (...,PDP::Swap(Get<Index>(a),Get<Index>(b)));
    }
}
...
auto ConcatenateTuple(Tuple1&& tup1, Tuple2&& tup2)
{
    return Detail::tuple_cat1(
            PDP::Forward<Tuple1>(tup1),
            PDP::Forward<Tuple2>(tup2),
            typename PDP::MakeIndexSequence<TupleSize<typename PDP::Decay<Tuple1>::type>::value>::type{},
            typename PDP::MakeIndexSequence<TupleSize<typename PDP::Decay<Tuple2>::type>::value>::type{}
    );
}
...
namespace LD
{
    ...
    struct  IsMoveConstructible
#ifndef _LIBCPP_HAS_NO_RVALUE_REFERENCES
            : public LD::IsConstructible<_Tp, typename LD::Detail::AddRvalueReference<_Tp>::type>
#else
        : public LD::IsCopyConstructible<_Tp>
#endif
    {};

#if _LIBCPP_STD_VER > 14 && !defined(_LIBCPP_HAS_NO_VARIABLE_TEMPLATES)
    template <class _Tp>  bool IsMoveConstructible_V
            = IsMoveConstructible<_Tp>::value;
#endif
}
