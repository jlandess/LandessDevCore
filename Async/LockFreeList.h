...
//  DataStructures
//
//  Created by James Landess on 1/28/18.
...
private:
    ...
    private:
        ...
        //CriticalSection m_sync_compare_and_swap;
        PDP::SpinLock CurrentLock;
    ...
    public:
        ...
        bool is_aux_cell()        { return ( (data == NULL) && is_not_basenode() && is_not_lastnode() ); }
        bool is_normal_cell()    { return ( data != NULL ); }
        
        bool is_basenode()        { return ( is_basenode_bool ); }
        bool is_lastnode()        { return ( is_lastnode_bool ); }
        bool is_not_basenode() { return ( is_basenode_bool == false ); }
        bool is_not_lastnode() { return ( is_lastnode_bool == false ); }
        
        // **********   set node status member functions   *********
        // These member functions set the node status.
        bool set_is_basenode() { is_basenode_bool = true; return true;}
        bool set_is_lastnode() { is_lastnode_bool = true; return true;}
        ...
        /**
         @param s - the first node to compare
         @param n - the new node to compare to
         @return - If the compare and swap was successful
         @brief Instead of utilizing the atomic primitives directly we're deferring it to the usage of the PDP::Multithreading::SpinLock which helps take advantage of primitives when they're available and it simply spins when they're not available
         */
        bool compare_and_swap_next(     list_node * s,     list_node * n )
        {
            ...
            /*
             ...
             Lock lock(&m_sync_compare_and_swap);
             // compare
             if ( next == s ) {
             // and swap
             next = n;
    ...
    std::atomic<int> ListSize;  // List must mutex this too. (list will own this lock)
    //CriticalSection m_sync_ListSize;
    
    // Lock counter and increment the list size.
    bool AddToListSize(int adder)
    {
        //Lock lock(&m_sync_ListSize);
        ListSize += adder;
        return true;
    }
...
public:
    ...
    // *****      get functions     *****
    list_node get_lastnode() { return lastnode;}
    ...
    public:
        ...
        bool go_next()
        {
            ...
            } // substitute for testing for lastnode
        ...
        list_node * saferead(list_node * node_ptr)
        {
            // Page 61 Valois
    ...
    // This function actually deletes the node.
    bool delete_node(iterator i)
    {
        ...
        if(r != true) {return false;} // if unsuccessful say so
        //----------------------------------------------------
        // Decrement list size and return.
        if (r == true) { AddToListSize(-1); return true;}
    ...
    void update_iterator(iterator * itr) { // update the iterator
        ...
        // if the pre_aux = target then fine and dandy
        if ( itr->pre_aux->next == itr->target ) { return; }
    ...
    void test_list2(int number_of_unique_values) {
        ...
        /*
         ...
         and "total_aux_cells").*/
        std::cout<<"Integrity test of list :";
        ...
        std::cout<<  "List internal add/delete counter: ListSize = "<<ListSize<<"\n";
        std::cout<<"\ntotal_aux_cells                            = "<<total_aux_cells<<"\n";
        
        // Report value counts.
        int i;
        for( i = 1 ; i < number_of_unique_values + 1 ; i++ ) {
        ...
        while(testnode) {
            ...
            if(testnode->is_valid() &&(testnode->is_normal_cell()) ) {
                std::cout<<"is a normal cell and its value is "<<*(testnode->data)<<"\n";
                total_normal_cells++;
                
                value_count[*(testnode->data)]=value_count[*(testnode->data)]+1;
            }
            // Test and output node status.
            if(testnode->is_aux_cell()) {std::cout<<"is an auxiliary cell.  \n";  total_aux_cells++;}
            if(testnode->is_basenode() ) {std::cout<<"is the basenode.  \n";}
            if(testnode->is_lastnode()) {std::cout<<"is the lastnode.  \n";}
