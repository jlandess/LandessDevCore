...
// ----------------------------------------------------------------------------
...

        {
            ...
            _inner = PDP::Move(PDP::MakeUnique<InnerControl>(*this));

	
            _thread = PDP::MakeUnique<Thread>([this, fun]
            {
...
// ----------------------------------------------------------------------------
...

        {
            ...
            //auto it = std::remove_if(_list.begin(), _list.end(),[&](const auto& cr_ptr) { return cr_ptr == nullptr; } );
	
            //_list.erase(it, _list.end());
            
            std::vector<std::shared_ptr<Coroutine>>::iterator i = _list.begin();
            while (i != _list.end())
            {
                
                if ((*i) == nullptr)
                {
                    _list.erase(i++);  // alternatively, i = items.erase(i);
                }
            }
