...
private:
  ...
  // The type of the buckets container
    ...
    using buckets_t = libcuckoo_bucket_container<Key, T, Allocator, partial_t, SLOT_PER_BUCKET>;
...
public:
   ...
   */
    ...
    cuckoohash_map(InputIt first, InputIt last,
                 ...
                 const KeyEqual &equal = KeyEqual(),
                 const Allocator &alloc = Allocator()): cuckoohash_map(n, hf, equal, alloc)
   ...
   */
    ...
    void swap(cuckoohash_map &other) noexcept
    {
        ...
        minimum_load_factor_.exchange(other.minimum_load_factor(),std::memory_order_release),std::memory_order_release);
...
private:
  ...
  // lookup would probably slow us down
    ...
    static constexpr bool is_simple = std::is_pod<key_type>::value && sizeof(key_type) <= 8;
  ...
  // properly.
    ...
    public:
        ...
        TwoBuckets(locks_t &locks, size_type i1_, size_type i2_, normal_mode): i1(i1_), i2(i2_), first_manager_(&locks[lock_ind(i1)]),
  ...
  // throws hashpower_changed if it changed after taking the lock.
    ...
    TwoBuckets lock_two(size_type hp, size_type i1, size_type i2,locked_table_mode) const
    {
  ...
  // are released. No meaningful position is returned.
    ...
    void add_to_bucket(const size_type bucket_ind, const size_type slot,const partial_t partial, K &&key, Args &&... val)
    {
  ...
  template <typename K>
  bool try_find_insert_bucket(const bucket &b, int &slot,const partial_t partial, const K &key) const
    {
    // Silence a warning from MSVC about partial being unused if is_simple.
  ...
  // throws hashpower_changed if it changed during the move.
    ...
    bool cuckoopath_move(const size_type hp, CuckooRecords &cuckoo_path, size_type depth, TwoBuckets &b)
    {
      ...
      // the cuckoopath is still valid.
            ...
            if (tb.occupied(ts) || !fb.occupied(fs) || hashed_key_only_hash(fb.key(fs)) != from.hv.hash)
            {
...
#pragma pack(push, 1)
    ...
    // be less than MAX_BFS_PATH_LEN, and also able to hold negative values.
        ...
        b_slot(const size_type b, const size_type p, const decltype(depth) d): bucket(b), pathcode(p), depth(d)
        {
...
#pragma pack(pop)
  ...
  class b_queue
    ...
    private:
        ...
        static_assert((MAX_CUCKOO_COUNT & (MAX_CUCKOO_COUNT - 1)) == 0,"MAX_CUCKOO_COUNT should be a power of 2");
  ...
  // provides a strong exception guarantee.
    ...
    cuckoo_status cuckoo_fast_double(size_type current_hp)
    {
        ...
        if (!std::is_nothrow_move_constructible<key_type>::value ||!std::is_nothrow_move_constructible<mapped_type>::value)
        {
    ...
    // elements from the old container to the new one.
        ...
        parallel_exec(0, hashsize(current_hp),[this, &new_buckets, current_hp, new_hp](size_type start, size_type end,std::exception_ptr &eptr)
        {
    ...
    void move_buckets(buckets_t &new_buckets, size_type current_hp,size_type new_hp, size_type start_ind, size_type end_ind)
    {
         ...
         ++old_bucket_ind)
        {
            ...
            for (size_type old_bucket_slot = 0; old_bucket_slot < slot_per_bucket();++old_bucket_slot)
            {
                ...
                const size_type old_ahash =alt_index(current_hp, hv.partial, old_ihash);
  ...
  // throws an exception, depending on the error type.
    ...
    cuckoo_status check_resize_validity(const size_type orig_hp,const size_type new_hp)
    {
  ...
  // them, so that old threads can resume and potentially re-start.
    ...
    void maybe_resize_locks(size_type new_bucket_count)
    {
        ...
        if (!(current_locks.size() < kMaxNumLocks &&current_locks.size() < new_bucket_count))
        {
        ...
        locks_t new_locks(std::min(size_type(kMaxNumLocks), new_bucket_count),spinlock(), get_allocator());
  ...
  // Executes the function over the given range split over num_threads threads
    ...
    {
    static const size_type num_threads = std::max(std::thread::hardware_concurrency(), 1U);
        ...
        std::vector<std::exception_ptr, rebind_alloc<std::exception_ptr>> eptrs(num_threads, nullptr, get_allocator());
...
public:
   ...
   */
     ...
     */
      ...
      // location, false otherwise.
            ...
            bool operator==(const const_iterator &it) const
            {
                ...
                return buckets_ == it.buckets_ && index_ == it.index_ &&slot_ == it.slot_;
      ...
      // step forward to the next data item, or to the end of the table.
            ...
            const_iterator(buckets_t &buckets, size_type index,size_type slot) noexcept: buckets_(std::addressof(buckets)), index_(index), slot_(slot)
            {
                ...
                if (std::make_pair(index_, slot_) != end_pos(*buckets_) &&!(*buckets_)[index_].occupied(slot_))
                {
     ...
     */
        ...
        public:
            ...
            reference operator*()
            {
                ...
                return (*const_iterator::buckets_)[const_iterator::index_].kvpair(const_iterator::slot_);
    ...
    // want users calling it.
        ...
        locked_table(cuckoohash_map &map) noexcept: map_(map),
 ...
 */
        ...
        void swap(cuckoohash_map<Key, T, Hash, KeyEqual, Allocator, SLOT_PER_BUCKET> &lhs,cuckoohash_map<Key, T, Hash, KeyEqual, Allocator, SLOT_PER_BUCKET>
        &rhs) noexcept
        {
