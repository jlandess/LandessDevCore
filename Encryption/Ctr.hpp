...
#ifndef LANDESSDEVCORE_CTR_HPP
#define LANDESSDEVCORE_CTR_HPP
#include "Crypt.hpp"
inline int ctr_start(int cipher, unsigned char *count, unsigned char *key, int keylen, int num_rounds, struct symmetric_CTR *ctr)
{
    int x;
...
inline void ctr_encrypt(unsigned char *pt, unsigned char *ct, int len, struct symmetric_CTR *ctr)
{
...
inline void ctr_decrypt(unsigned char *ct, unsigned char *pt, int len, struct symmetric_CTR *ctr)
{
    ctr_encrypt(ct, pt, len, ctr);
}
#endif //LANDESSDEVCORE_CTR_HPP
