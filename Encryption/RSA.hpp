...
#ifndef LANDESSDEVCORE_RSA_HPP
#define LANDESSDEVCORE_RSA_HPP
#include "Crypt.hpp"
...
*/
/*
inline int rsa_encrypt(unsigned char *in, int len, unsigned char *out, int *outlen,
...
inline int rsa_decrypt(unsigned char *in, int len, unsigned char *out, int *outlen, struct rsa_key *key)
{
    unsigned char sym_IV[32], sym_key[32], rsa_in[4096], rsa_out[4096];
    struct symmetric_CTR ctr;
...
inline int rsa_sign(unsigned char *in, int inlen, unsigned char *out, int *outlen, int hash,
...
inline int rsa_verify(unsigned char *sig, unsigned char *msg, int inlen, int *stat, struct rsa_key *key)
{
