...
#ifndef LANDESSDEVCORE_CBC_HPP
#define LANDESSDEVCORE_CBC_HPP
#include "Crypt.hpp"

inline int cbc_start(int cipher, unsigned char *IV, unsigned char *key, int keylen, int num_rounds, struct symmetric_CBC *cbc)
{
    int x;
...
inline void cbc_encrypt(unsigned char *pt, unsigned char *ct, struct symmetric_CBC *cbc)
{
    int x;
...
inline void cbc_decrypt(unsigned char *ct, unsigned char *pt, struct symmetric_CBC *cbc)
{
    int x;
    unsigned char tmp[32];
