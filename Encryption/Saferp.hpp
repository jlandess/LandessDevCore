...
#ifndef LANDESSDEVCORE_SAFERP_HPP
#define LANDESSDEVCORE_SAFERP_HPP
#include "Crypt.hpp"
#define ROUND(b, i) 										\
...
inline int saferp_setup(unsigned char *key, int keylen, int num_rounds, union symmetric_key *skey)
{
    unsigned x, y;
    unsigned char t[33];
...
inline void saferp_ecb_encrypt(unsigned char *pt, unsigned char *ct, union symmetric_key *skey)
{
    unsigned char b[16];
...
inline void saferp_ecb_decrypt(unsigned char *ct, unsigned char *pt, union symmetric_key *skey)
{
    unsigned char b[16];
...
#endif //LANDESSDEVCORE_SAFERP_HPP
